...
const debug = debugFactory('loopback:grpc:proto-manager');
...
export class ProtoManager {
  ...
  private generate(proto: string): Buffer {
    ...
    return execSync(
      `${protocPath} --plugin=protoc-gen-ts=${pluginPath}
 --ts_out=generate_package_definition:service=true:${root} -I ${root} ${proto}`,
    );
  }
}
